...
EkmanUWCurrentModel::EkmanUWCurrentModel(int i) : seabed(i),
                                                    rho(1),   
                                                    dTop(0), 
                                                    dBottom(1), 
                                                    velocity(0), 
...
EkmanUWCurrentModel::Input EkmanUWCurrentModel::parse(const std::string& yaml_input)
{
    ...
    double wind_angle;
    if (node.FindValue("wind angle")) {ssc::yaml_parser::parse_uv(node["wind angle"], wind_angle);}
    else {wind_angle = 0;}

    double U10;
    if (node.FindValue("U10")) {ssc::yaml_parser::parse_uv(node["U10"], U10);}
    else {U10 = 1.;}

    double rho_air;
    if (node.FindValue("rho air")) {node["rho air"] >> rho_air;}
    else {rho_air = 1.225;}

    if (node.FindValue("velocity")) {ssc::yaml_parser::parse_uv(node["velocity"], ret.velocity);}
    else {ret.velocity = 1.;}

    if (node.FindValue("phi")) {node["phi"] >> ret.phi;}
    else {ret.phi = 48;}

    if (node.FindValue("rho")) {node["rho"] >> ret.rho;}
    else {ret.rho = 1026;}

    if (node.FindValue("omega")) {node["omega"] >> ret.omega;}
    else {ret.omega = 7.2921e-5;} // rad/s

    if (node.FindValue("dTop")) {node["dTop"] >> ret.dTop;}
    else {ret.dTop = 10;}

    if (node.FindValue("dBottom")) {node["dBottom"] >> ret.dBottom;}
    else {ret.dBottom = 100;}
...
EkmanUWCurrentModel::Input::Input() : seabed(100.), rho(), dTop(), dBottom(), velocity(), phi(), omega(), windTau()
{
}

EkmanUWCurrentModel::Input::Input(Seabed seabed) : seabed(seabed), rho(), dTop(), dBottom(), velocity(), phi(), omega(), windTau()
{
}