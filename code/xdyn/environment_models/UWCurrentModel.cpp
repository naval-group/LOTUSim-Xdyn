...
UWCurrentModel::Seabed::Seabed(std::string sea_bed_file)
    : seabed_file(sea_bed_file)
    , seabed()
    , xmin()
    , xmax()
    , ymin()
...
double UWCurrentModel::Seabed::get_height(const double x, const double y) const
{
    ...
    int xp = (int)std::ceil(xx);
    int xm = (int)std::floor(xx);
    int yp = (int)std::ceil(yy);
    int ym = (int)std::floor(yy);
    double x2 = xp - xx;
    double x1 = xx - xm;
    double y2 = yp - yy;
    double y1 = yy - ym;
