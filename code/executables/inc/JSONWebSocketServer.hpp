...
{
public:
    JSONWebSocketServer(const ServiceT& service, const bool verbose):
...
private:
    ...
    {
    public:
        JSONHandler(const ServiceT& simserver, const bool verbose_):
        ...
        void operator()(const ssc::websocket::Message& msg)
        {
            ...
            const std::function<void(const std::string&)> quiet_error_outputter = [&msg, this](const std::string& what) {msg.send_text(replace_newlines_by_spaces(std::string("{\"error\": \"") + what + "\"}"));};
            const std::function<void(const std::string&)> verbose_error_outputter = [&msg, this](const std::string& what) {std::cerr << current_date_time() << " Error: " << what << std::endl; msg.send_text(replace_newlines_by_spaces(std::string("{\"error\": \"") + what + "\"}"));};
